...
import { Send, MessageCircle, Mail, Linkedin, Download } from "lucide-react"
...
export function DevContact() {
  ...
  const t = translations.dev[lang].contact
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ name, email, message, lang, source: "dev" }),
      })
      ...
      <div ref={ref} className="max-w-2xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 flex-wrap">
            <a
              href="https://wa.me/5527988770632?text=Oi%20Antonio!%20Vi%20seu%20portfolio%20dev%20e%20quero%20conversar."
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 bg-card border border-border rounded-lg px-5 py-3 text-sm text-foreground hover:border-emerald-500/40 hover:text-emerald-400 transition-colors"
            >
            ...
            <a
              ...
              className="flex items-center gap-2 bg-card border border-border rounded-lg px-5 py-3 text-sm text-foreground hover:border-primary/40 hover:text-primary transition-colors"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 bg-card border border-border rounded-lg px-5 py-3 text-sm text-foreground hover:border-sky-500/40 hover:text-sky-400 transition-colors"
            >
